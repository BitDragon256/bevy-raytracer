    ...
    for (mut mesh, material, transform) in &mut meshes {
        ...
        all_meshes.push(GpuNEMesh {
            ...
            surface_area: mesh.faces.iter().map(|f| {
                let a = transform.apply(mesh.vertices[f.a as usize].pos);
                let b = transform.apply(mesh.vertices[f.b as usize].pos);
                let c = transform.apply(mesh.vertices[f.c as usize].pos);
